...
export default function Home() {
  ...
  const { playlists, loading, error, updatePlaylist, deletePlaylist } = usePlaylists(user?.uid);
  const {
    filters,
    filteredPlaylists,
    updateFilter,
  ...
  const handleMarkAllComplete = async () => {
    ...
    try {
      for (const playlist of selectedPlaylists) {
        const updatedVideos = playlist.videos.map(video => ({ ...video, completed: true }));
        const completedDuration = playlist.totalDuration; // All videos completed
  ...
  const handleMarkAllIncomplete = async () => {
    ...
    try {
      for (const playlist of selectedPlaylists) {
        const updatedVideos = playlist.videos.map(video => ({ ...video, completed: false }));
  ...
  const handleDeleteSelected = async () => {
    const confirmMessage = `Are you sure you want to delete ${selectedCount} playlist${selectedCount > 1 ? 's' : ''}? This action cannot be undone.`;
  ...
  const handleToggleBulkMode = () => {
    ...
    if (showBulkMode) {
      selectNone(); // Clear selection when exiting bulk mode
    }
  };
  ...
  return (
    <div className="min-h-screen bg-transparent">
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        {/* Show search bar and bulk mode toggle only if there are playlists */}
        {!loading && playlists.length > 0 && (
          <>
            ...
            {/* Selection header - only show in bulk mode */}
            <SelectionHeader
              totalCount={filteredPlaylists.length}
              selectedCount={selectedCount}
              onToggleSelectAll={() => toggleSelectAll(filteredPlaylists.map(p => p.id))}
