...
export const FindFriendsModal: React.FC<FindFriendsModalProps> = ({
  ...
  const { friends, incomingRequests, sentRequests } = useFriends(
    currentUser?.uid
  );
  ...
  useEffect(() => {
    ...
    const search = async () => {
      setLoading(true);
      const users = await DatabaseService.searchUsers(debouncedQuery);
      setResults(users.filter((u: UserProfile) => u.uid !== currentUser?.uid));
      setLoading(false);
    };
  ...
  const getRelationshipStatus = (targetUserId: string) => {
    ...
    if (incomingRequests.some((r) => r.fromUid === targetUserId))
      return "Request Received";
    if (sentRequests.some((r) => r.toUid === targetUserId))
      return "Request Sent";
    return "None";
  };
  ...
  return (
    ...
    >
      <div
        className="glass-panel p-6 rounded-lg w-full max-w-md"
      ...
      >
        ...
        <div className="space-y-2 max-h-80 overflow-y-auto">
          {loading && (
            <p className="text-neutral-400 text-center">Searching...</p>
          )}
          {!loading &&
            results.map((user) => {
              ...
              return (
                ...
                >
                  ...
                  {status === "Request Sent" && (
                    <div className="p-2 text-neutral-400 flex items-center gap-1 text-sm">
